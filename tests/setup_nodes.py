...
from pipscoin.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from pipscoin.full_node.full_node_api import FullNodeAPI
from pipscoin.server.start_farmer import service_kwargs_for_farmer
from pipscoin.server.start_full_node import service_kwargs_for_full_node
from pipscoin.server.start_harvester import service_kwargs_for_harvester
from pipscoin.server.start_introducer import service_kwargs_for_introducer
from pipscoin.server.start_service import Service
from pipscoin.server.start_timelord import service_kwargs_for_timelord
from pipscoin.server.start_wallet import service_kwargs_for_wallet
from pipscoin.simulator.start_simulator import service_kwargs_for_full_node_simulator
from pipscoin.timelord.timelord_launcher import kill_processes, spawn_process
...
from pipscoin.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["pips_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "pips")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["pips_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "pips")
