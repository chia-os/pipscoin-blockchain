...
from pipscoin.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from pipscoin.protocols import full_node_protocol
from pipscoin.protocols.full_node_protocol import RespondBlock
from pipscoin.rpc.rpc_server import start_rpc_server
from pipscoin.rpc.wallet_rpc_api import WalletRpcApi
from pipscoin.rpc.wallet_rpc_client import WalletRpcClient
from pipscoin.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
from pipscoin.wallet.derive_keys import master_sk_to_local_sk
...
class TestPoolWalletRpc:
    ...
    def create_pool_plot(self, p2_singleton_puzzle_hash: bytes32, shuil=None) -> bytes32:
        ...
        args.pool_contract_address = encode_puzzle_hash(p2_singleton_puzzle_hash, "tpips")
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc):
        ...
        # Balance stars at 6 PIPS and 5 more blocks are farmed, total 22 PIPS
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            async def have_pipscoin():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_pipscoin)
