...
from pipscoin.plotting.plot_tools import stream_plot_info_ph, stream_plot_info_pk
from pipscoin.protocols import farmer_protocol
from pipscoin.rpc.farmer_rpc_api import FarmerRpcApi
from pipscoin.rpc.farmer_rpc_client import FarmerRpcClient
from pipscoin.rpc.harvester_rpc_api import HarvesterRpcApi
from pipscoin.rpc.harvester_rpc_client import HarvesterRpcClient
from pipscoin.rpc.rpc_server import start_rpc_server
...
from pipscoin.wallet.derive_keys import master_sk_to_wallet_sk, master_sk_to_pooling_authentication_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "pips"), encode_puzzle_hash(new_ph_2, "pips"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "pips"))
            ...
            assert config["farmer"]["pips_target_address"] == encode_puzzle_hash(new_ph, "pips")
            assert config["pool"]["pips_target_address"] == encode_puzzle_hash(new_ph_3, "pips")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "pips")
