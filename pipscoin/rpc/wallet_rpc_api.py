...
from pipscoin.cmds.init_funcs import check_keys
from pipscoin.consensus.block_rewards import calculate_base_farmer_reward
from pipscoin.pools.pool_wallet import PoolWallet
from pipscoin.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from pipscoin.protocols.protocol_message_types import ProtocolMessageTypes
from pipscoin.server.outbound_message import NodeType, make_msg
...
from pipscoin.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from pipscoin.util.byte_types import hexstr_to_bytes
from pipscoin.util.ints import uint32, uint64
from pipscoin.util.keychain import bytes_to_mnemonic, generate_mnemonic
from pipscoin.util.path import path_from_root
from pipscoin.util.ws_message import WsRpcMessage, create_payload_dict
from pipscoin.wallet.cc_wallet.cc_wallet import CCWallet
from pipscoin.wallet.derive_keys import master_sk_to_singleton_owner_sk
from pipscoin.wallet.rl_wallet.rl_wallet import RLWallet
from pipscoin.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from pipscoin.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from pipscoin.wallet.util.trade_utils import trade_record_to_dict
...
from pipscoin.wallet.wallet_info import WalletInfo
from pipscoin.wallet.wallet_node import WalletNode
from pipscoin.util.config import load_config
from pipscoin.consensus.coinbase import create_puzzlehash_for_pk
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "pipscoin_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "pipscoin_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("pips_target_address")
        pool_target = config["pool"].get("pips_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from pipscoin.pools.pool_wallet_info import initial_pool_state_from_dict
