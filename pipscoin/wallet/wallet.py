...
from pipscoin.consensus.cost_calculator import calculate_cost_of_program, NPCResult
from pipscoin.full_node.bundle_tools import simple_solution_generator
...
from pipscoin.types.blockchain_format.program import Program, SerializedProgram
from pipscoin.types.announcement import Announcement
...
from pipscoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from pipscoin.wallet.puzzles.puzzle_utils import (
...
from pipscoin.wallet.secret_key_store import SecretKeyStore
from pipscoin.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_pipscoin(self, pipscoin_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if pipscoin_amount < 0:
            utxos = await self.select_coins(abs(pipscoin_amount), exclude)
        else:
        ...
        pipscoin_amount = spend_value + pipscoin_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": pipscoin_amount}]
