...
from pipscoin.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from pipscoin.protocols import wallet_protocol
from pipscoin.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from pipscoin.protocols.protocol_message_types import ProtocolMessageTypes
from pipscoin.protocols.wallet_protocol import (
...
from pipscoin.util.errors import Err, ValidationError
from pipscoin.util.ints import uint32, uint128
from pipscoin.util.keychain import Keychain
from pipscoin.util.lru_cache import LRUCache
from pipscoin.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from pipscoin.wallet.settings.settings_objects import BackupInitialized
...
from pipscoin.wallet.wallet_blockchain import ReceiveBlockResult
from pipscoin.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[PipscoinServer]
    log: logging.Logger
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'pipscoin keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSPipscoinConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSPipscoinConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSPipscoinConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSPipscoinConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSPipscoinConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSPipscoinConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
