...
def get_pipscoin_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "pipscoin_ca.crt")
    key = pkg_resources.resource_string(__name__, "pipscoin_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Pipscoin"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Pipscoin"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("pipscoin.net")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Pipscoin"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Pipscoin CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def main():
    return make_ca_cert(Path("./pipscoin_ca.crt"), Path("./pipscoin_ca.key"))
