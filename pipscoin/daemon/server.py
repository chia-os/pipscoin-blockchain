...
from pipscoin.cmds.init_funcs import pipscoin_init
from pipscoin.daemon.windows_signal import kill
from pipscoin.server.server import ssl_context_for_root, ssl_context_for_server
from pipscoin.ssl.create_ssl import get_mozilla_ca_crt
from pipscoin.util.pipscoin_logging import initialize_logging
...
from pipscoin.util.service_groups import validate_service
from pipscoin.util.setproctitle import setproctitle
from pipscoin.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Pipscoin.")
    quit()
...
service_plotter = "pipscoin plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "pipscoin": "pipscoin",
        "pipscoin_wallet": "start_wallet",
        "pipscoin_full_node": "start_full_node",
        "pipscoin_harvester": "start_harvester",
        "pipscoin_farmer": "start_farmer",
        "pipscoin_introducer": "start_introducer",
        "pipscoin_timelord": "start_timelord",
        "pipscoin_timelord_launcher": "timelord_launcher",
        "pipscoin_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("pipscoin daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered PIPSCOIN_ROOT
    os.environ["PIPSCOIN_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with PIPSCOIN_ROOT: {os.environ['PIPSCOIN_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    pipscoin_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("pipscoin_daemon")
