...
import pipscoin.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from pipscoin.consensus.block_creation import unfinished_block_to_full_block
from pipscoin.consensus.block_record import BlockRecord
from pipscoin.consensus.blockchain import Blockchain, ReceiveBlockResult
from pipscoin.consensus.constants import ConsensusConstants
from pipscoin.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from pipscoin.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from pipscoin.consensus.multiprocess_validation import PreValidationResult
from pipscoin.consensus.pot_iterations import calculate_sp_iters
from pipscoin.full_node.block_store import BlockStore
from pipscoin.full_node.bundle_tools import detect_potential_template_generator
from pipscoin.full_node.coin_store import CoinStore
from pipscoin.full_node.full_node_store import FullNodeStore
from pipscoin.full_node.mempool_manager import MempoolManager
from pipscoin.full_node.signage_point import SignagePoint
from pipscoin.full_node.sync_store import SyncStore
from pipscoin.full_node.weight_proof import WeightProofHandler
from pipscoin.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from pipscoin.protocols.full_node_protocol import (
    RejectBlocks,
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
...
from pipscoin.protocols.protocol_message_types import ProtocolMessageTypes
from pipscoin.server.node_discovery import FullNodePeers
from pipscoin.server.outbound_message import Message, NodeType, make_msg
from pipscoin.server.server import PipscoinServer
from pipscoin.types.blockchain_format.classgroup import ClassgroupElement
from pipscoin.types.blockchain_format.pool_target import PoolTarget
from pipscoin.types.blockchain_format.sized_bytes import bytes32
from pipscoin.types.blockchain_format.sub_epoch_summary import SubEpochSummary
from pipscoin.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
from pipscoin.types.end_of_slot_bundle import EndOfSubSlotBundle
from pipscoin.types.full_block import FullBlock
from pipscoin.types.header_block import HeaderBlock
from pipscoin.types.mempool_inclusion_status import MempoolInclusionStatus
from pipscoin.types.spend_bundle import SpendBundle
from pipscoin.types.unfinished_block import UnfinishedBlock
from pipscoin.util.bech32m import encode_puzzle_hash
from pipscoin.util.db_wrapper import DBWrapper
from pipscoin.util.errors import ConsensusError, Err
from pipscoin.util.ints import uint8, uint32, uint64, uint128
from pipscoin.util.path import mkdir, path_from_root
from pipscoin.util.safe_cancel_task import cancel_task_safe
from pipscoin.util.profiler import profile_task
...
class FullNode:
    ...
    def set_server(self, server: PipscoinServer):
        self.server = server
        dns_servers = []
        try:
        ...
        elif self.config["port"] == 8888:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.pipscoin.net")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSPipscoinConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSPipscoinConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSPipscoinConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSPipscoinConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSPipscoinConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSPipscoinConnection):
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSPipscoinConnection]
    ):
        """
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSPipscoinConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        peer: Optional[ws.WSPipscoinConnection],
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'pips')}, "
                f"validation time: {validation_time}, "
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSPipscoinConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSPipscoinConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSPipscoinConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSPipscoinConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSPipscoinConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
