...
from pipscoin.cmds.units import units
...
from pipscoin.util.misc import format_bytes
from pipscoin.util.misc import format_minutes
from pipscoin.util.network import is_localhost
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total pipscoin farmed: {amounts['farmed_amount'] / units['pipscoin']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['pipscoin']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['pipscoin']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'pipscoin start wallet' and 'pipscoin wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'pipscoin wallet show'")
    else:
        print("Note: log into your key using 'pipscoin wallet show' to see rewards for each key")
