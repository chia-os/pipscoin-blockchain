...
from pipscoin.ssl.create_ssl import generate_ca_signed_cert, get_pipscoin_ca_crt_key, make_ca_cert
from pipscoin.util.bech32m import encode_puzzle_hash
from pipscoin.util.config import (
    create_default_pipscoin_config,
    initial_config_file,
    load_config,
    save_config,
...
def check_keys(new_root: Path) -> None:
    ...
    if "pips_target_address" not in config["farmer"]:
        print(f"Setting the pips destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "pips_target_address" not in config["pool"]:
        print(f"Setting the pips destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    pipscoin_ca_crt, pipscoin_ca_key = get_pipscoin_ca_crt_key()
    pipscoin_ca_crt_path = ca_dir / "pipscoin_ca.crt"
    pipscoin_ca_key_path = ca_dir / "pipscoin_ca.key"
    pipscoin_ca_crt_path.write_bytes(pipscoin_ca_crt)
    pipscoin_ca_key_path.write_bytes(pipscoin_ca_key)
    ...
    pipscoin_ca_crt, pipscoin_ca_key = get_pipscoin_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, pipscoin_ca_crt, pipscoin_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if pipscoin_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return pipscoin_init(root_path)
...
def pipscoin_minor_release_number():
    res = int(pipscoin_version_number()[2])
    print(f"Install release number: {res}")
    return res


def pipscoin_full_version_str() -> str:
    major, minor, patch, dev = pipscoin_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def pipscoin_init(root_path: Path):
    if os.environ.get("PIPSCOIN_ROOT", None) is not None:
        print(
            f"warning, your PIPSCOIN_ROOT is set to {os.environ['PIPSCOIN_ROOT']}. "
            f"Please unset the environment variable and run pipscoin init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Pipscoin directory {root_path}")
