...
from pipscoin.daemon.server import singleton, service_launch_lock_path
from pipscoin.server.ssl_context import pipscoin_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"pipscoin_{service_name}"
        ...
        pipscoin_ca_crt, pipscoin_ca_key = pipscoin_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = PipscoinServer(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (pipscoin_ca_crt, pipscoin_ca_key),
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for PipscoinServer to be closed")
