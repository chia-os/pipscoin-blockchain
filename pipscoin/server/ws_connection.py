...
from pipscoin.cmds.init_funcs import pipscoin_full_version_str
from pipscoin.protocols.protocol_message_types import ProtocolMessageTypes
from pipscoin.protocols.shared_protocol import Capability, Handshake
...
from pipscoin.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from pipscoin.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSPipscoinConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            ...
            if inbound_handshake.network_id != 'pipscoin-' + network_id:
