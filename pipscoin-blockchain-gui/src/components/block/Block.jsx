...
export default function Block() {
  ...
  const poolReward = mojo_to_pipscoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_pipscoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const pipscoinFees = blockRecord.fees
    ? mojo_to_pipscoin(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.pipscoinexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.pipscoinexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: pipscoinFees ? `${pipscoinFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Pipscoin blockchain
            </Trans>
          </BlockTitle>
        }
        action={
