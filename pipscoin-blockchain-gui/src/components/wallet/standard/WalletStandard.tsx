...
import {
  ...
  Fee,
  Form,
  TextField as PipscoinTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
...
import { /* mojo_to_pipscoin_string, */ pipscoin_to_mojo } from '../../../util/pipscoin';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_pipscoin_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of pipscoin in the blockchain at the current
      ...
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the amount of Pipscoin that you can currently use to make
            transactions. It does not include pending farming rewards, pending
            incoming transactions, and Pipscoin that you have just spent but is not
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send pipscoin to coloured address. Please enter a pipscoin
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'pipscoin_addr://') {
    ...
    const amountValue = Number.parseFloat(pipscoin_to_mojo(amount));
    const feeValue = Number.parseFloat(pipscoin_to_mojo(fee));
