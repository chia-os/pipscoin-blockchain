...
import { AlertDialog, Card, Flex } from '@pipscoin/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hepipseck = /[\da-f]+$/gi;

    if (!hepipseck.test(ip_val) || ip_val.value === '') {
    ...
    const pipscoinper_input = ip_parsed.limit;
    ...
    const pipscoinper_value = Number.parseInt(Number(pipscoinper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (pipscoin per interval):{' '}
                  {mojo_to_pipscoin_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = pipscoin_to_mojo(fee_input.value);
