...
import { AlertDialog } from '@pipscoin/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      pipscoinper_input.value === '' ||
      Number(pipscoinper_input.value) === 0 ||
      !Number(pipscoinper_input.value) ||
      isNaN(Number(pipscoinper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
