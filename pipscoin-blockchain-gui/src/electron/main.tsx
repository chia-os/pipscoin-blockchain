...
import pipscoinEnvironment from '../util/pipscoinEnvironment';
import pipscoinConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!pipscoinEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    pipscoinConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!pipscoinConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !pipscoinConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (pipscoinConfig.manageDaemonLifetime()) {
        pipscoinEnvironment.startPipscoinDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Pipscoin Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Pipscoin-Network/pipscoin-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Pipscoin-Network/pipscoin-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Pipscoin-Network/pipscoin-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Pipscoin-Network/pipscoin-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Pipscoin-Network/pipscoin-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/pipscoin_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/pipscoin_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Pipscoin Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Pipscoin' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Pipscoin Blockchain' }),
            click: () => {
              openAbout();
            },
          },
