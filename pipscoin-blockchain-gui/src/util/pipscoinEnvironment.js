...
const PY_FOLDER = '../pipscoin/daemon';
...
const getPipscoinVersion = () => {
  let version = null;
  const exePath = getExecutablePath('pipscoin');
  // first see if we can get a pipscoin exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or pipscoin is on the path
    try {
      version = child_process
